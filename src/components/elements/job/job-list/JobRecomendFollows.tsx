...
import { getRecommendJobsByFollowedEmployers } from '@/apis/jobAPI';

export default function JobRecommendFollows() {
  ...
  const fetchJobsBanner = async () => {
    try {
      const data = await getRecommendJobsByFollowedEmployers();
      setJobsBanner(data);
    } catch (error) {
      console.error('Lỗi khi tải danh sách việc làm tuyển gấp');
    }
  };
